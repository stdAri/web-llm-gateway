...
- 2026-09-02 — Live leg verified, retiring the fake-Bridge caveat above: real Claude Code 2.1.257, configured with only `ANTHROPIC_BASE_URL` + `ANTHROPIC_AUTH_TOKEN` and `--model deepseek/deepseek-chat`, completed a Q&A turn against the real DeepSeek web conversation and returned "中国的首都是北京，编号42。" in 6.1s — the requested marker proves the answer was genuinely generated by the web model. (Claude Code's "unrecognized model" warning remains cosmetic.)

- 2026-09-03 — Criterion 5 re-verified with real Claude Code rather than a fake Bridge, which is how it was originally signed off. `claude -p` configured with only `ANTHROPIC_BASE_URL` and `ANTHROPIC_AUTH_TOKEN` answered a question through DeepSeek, and the exchange was confirmed present in the live tab, so the answer demonstrably came from the Web Product rather than from Anthropic.
